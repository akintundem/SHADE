...
class BaseAgent(ABC):
    ...
    async def process_message(self, message: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            system_prompt_fn = getattr(self, 'get_system_prompt')
            try:
    ...
    async def execute_tool(self, tool_name: str, tool_args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Find the tool by name, supporting both LangChain tools and bare callables
            def _tool_name(t):
                return getattr(t, 'name', getattr(t, '__name__', None))
            tool = next((t for t in self.tools if _tool_name(t) == tool_name), None)
            if not tool:
                return {"error": f"Tool {tool_name} not found"}
            
            # Execute the tool (prefer ainvoke/run if available, else call callable)
